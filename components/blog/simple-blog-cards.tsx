...
interface SimpleBlogCardsProps {
  title: string;
  blogCards:any[];
}
export default function SimpleBlogCards({ blogCards, title }: SimpleBlogCardsProps) {
  return (
    <div className="mb-10">
      ...
      <div className="grid lg:grid-cols-3 grid-cols-1 gap-x-4 xl:gap-x-0 gap-y-4">
        {blogCards.map((card, index) => (
          <Link key={index} href={`/blog/${card.slug?.current || card.slug || '#'}`} className="block">
            <div className="shadow-sm group hover:cursor-pointer bg-white max-w-[385px] rounded-[4px] hover:shadow-md transition-shadow duration-300">
